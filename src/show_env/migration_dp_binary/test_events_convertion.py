...
from ..show_px4.drone_px4.drone_px4 import ColorEvents, FireEvents, PositionEvents
from .events_convertion import decode_events, encode_events
...
def test_encode_decode_position_events(
    ...
    position_events.add_timecode_xyz(
        timecode=first_timecode, xyz=(first_x, first_y, first_z)
    )
    position_events.add_timecode_xyz(
        timecode=second_timecode, xyz=(second_x, second_y, second_z)
    )
...
def test_encode_decode_color_events(
    ...
    color_events.add_timecode_rgbw(
        timecode=first_timecode, rgbw=(first_r, first_g, first_b, first_w)
    )
    color_events.add_timecode_rgbw(
        timecode=second_timecode, rgbw=(second_r, second_g, second_b, second_w)
    )
...
def test_encode_decode_fire_events(
    ...
    fire_events.add_timecode_chanel_duration(
        timecode=first_timecode, chanel=first_chanel, duration=first_duration
    )

    fire_events.add_timecode_chanel_duration(
        timecode=second_timecode, chanel=second_chanel, duration=second_duration
    )
