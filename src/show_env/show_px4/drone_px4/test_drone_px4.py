...
def test_add_position_standard_case() -> None:
    ...
    assert drone.position_events[0].timecode == 0
    assert drone.position_events[0].xyz == (0, 0, 0)
    drone.add_position(timecode=1, xyz=(1, 1, 1))
    assert drone.position_events[1].timecode == 1
    assert drone.position_events[1].xyz == (1, 1, 1)
    drone.add_position(timecode=2, xyz=(2, 2, 2))
    assert drone.position_events[2].timecode == 2
    assert drone.position_events[2].xyz == (2, 2, 2)


def test_add_color_standard_case():
    drone = DronePx4(index=0)
    drone.add_color(timecode=0, rgbw=(0, 0, 0, 0))
    assert drone.color_events._events[0].timecode == 0
    assert drone.color_events._events[0].rgbw == (0, 0, 0, 0)
    drone.add_color(timecode=1, rgbw=(1, 1, 1, 1))
    assert drone.color_events._events[1].timecode == 1
    assert drone.color_events._events[1].rgbw == (1, 1, 1, 1)
    drone.add_color(timecode=2, rgbw=(2, 2, 2, 2))
    assert drone.color_events._events[2].timecode == 2
    assert drone.color_events._events[2].rgbw == (2, 2, 2, 2)


def test_add_fire_standard_case():
    drone = DronePx4(index=0)
    drone.add_fire(timecode=0, chanel=0, duration_frame=0)
    assert drone.fire_events._events[0].timecode == 0
    assert drone.fire_events._events[0].chanel == 0
    assert drone.fire_events._events[0].duration == 0
    drone.add_fire(timecode=1, chanel=1, duration_frame=1)
    assert drone.fire_events._events[1].timecode == 1
    assert drone.fire_events._events[1].chanel == 1
    assert drone.fire_events._events[1].duration == 1
    drone.add_fire(timecode=2, chanel=2, duration_frame=2)
    assert drone.fire_events._events[2].timecode == 2
    assert drone.fire_events._events[2].chanel == 2
    assert drone.fire_events._events[2].duration == 2
