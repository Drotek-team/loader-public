...
def apply_show_duration_logic_check_report(
    ...
    if (
        int(frame_parameter.json_fps * drones_simulation.duration)
        != show_configuration.duration
    ):
        show_duration_logic_check_report.update_report(
            drones_simulation.duration, show_configuration.duration
        )
        return
...
def apply_altitude_range_logic_check_report(
    ...
    if (
        drones_simulation.altitude_range[0] != show_configuration.altitude_range[0]
        and drones_simulation.altitude_range[1] != show_configuration.altitude_range[1]
    ):
        altitude_range_logic_check_report.update_report(
            drones_simulation.altitude_range, show_configuration.altitude_range
        )
        return
