...
def test_position_event_standard_case_and_method(magic_number: MagicNumber) -> None:
    position_event = PositionEvent(frame=0, x=1, y=2, z=3, scale=1)
    ...
    assert position_event.scale == 1
    assert position_event.get_data(magic_number) == [0, 1, 2, 3]


@pytest.mark.parametrize("magic_number", list(MagicNumber))
def test_position_event_standard_case_and_method_scale(magic_number: MagicNumber) -> None:
    position_event = PositionEvent(frame=0, x=1, y=2, z=3, scale=2)
    assert position_event.frame == 0
    assert position_event.x == 1
    assert position_event.y == 2
    assert position_event.z == 3
    assert position_event.xyz == (1, 2, 3)
    assert position_event.get_data(magic_number) == [0, 0, 1, 2]

    position_event = PositionEvent(frame=0, x=1, y=2, z=3, scale=3)
    assert position_event.get_data(magic_number) == [0, 0, 1, 1]

    position_event = PositionEvent(frame=0, x=1, y=2, z=3, scale=4)
    assert position_event.get_data(magic_number) == [0, 0, 0, 1]

    position_event = PositionEvent(frame=0, x=256, y=512, z=768, scale=4)
    assert position_event.get_data(magic_number) == [0, 64, 128, 192]


@pytest.mark.parametrize("magic_number", list(MagicNumber))
def test_position_events_standard_case_and_method(magic_number: MagicNumber) -> None:
    position_events = PositionEvents(magic_number, scale=1)
